...
つまり、**同じWebアプリURLで予約API・Messaging API・メモリーLIFF画面を併用**できます。

- **GET** でクエリ無し（`action` なし）→ メモリー用 `MemoryPage.html`（LIFFのエンドポイントURLにこの `/exec` を指定）
- **GET** `?action=photos&lineUserId=...` → メモリー写真一覧（JSON・LIFF内から取得）
...
| `MEMBER_PHOTO_DRIVE_FOLDER_ID` | ✅（写真運用時） | メモリー写真の保存先Google DriveフォルダID |
| `MEMORY_LIFF_ID` | ✅（メモリーLIFF利用時） | メモリー専用LIFF ID（`HtmlService` 埋め込み用。チャットと同じチャネルで別LIFFを追加） |
...
| `Log` | 会話ログ（横持ち） |
| `ErrorLog` | 失敗イベント |
| `MemberPhotoLog` | メモリー写真の保存ログ（userId、表示名、保存日、失効日、Drive、サムネURL 等） |
...
3. 「Webhookの利用」をON
4. 「検証」実行で200が返ることを確認
5. **メモリーLIFF** を追加: エンドポイントURL は **上記と同じ** `/exec`（予約用LIFFとは別IDで作成する）
6. リッチメニュー「メモリー」→ `https://liff.line.me/{MEMORY_LIFF_ID}`

### メモリーが「読み込み中」のままになるとき

- **GASを再デプロイ**したか確認する（`コード.gs` と `MemoryPage.html` 両方）
- **スクリプトプロパティの `SPREADSHEET_ID`** が、実際に `MemberPhotoLog` を開いているスプレッドシートと同じIDか確認する（ここが違うと一覧は常に0件）
- `MemberPhotoLog` は **旧8列**（status が H 列）でも **新10列** でも読み取れる
...
## メモリー写真（LINE画像 → Drive保存 → 同GASのLIFFで表示）

お客様がLINEトークで画像を送ると、`lineUserId` に紐づけてGoogle Driveへ保存します。表示は **GASプロジェクトに追加した `MemoryPage`（HTML）** を LIFF で開きます（Vercel不要）。

### GASファイル構成（もりもり）

- `コード.gs`（本リポジトリの `gas/Code.gs` を貼り付け）
- `MemoryPage.html`（本リポジトリの `gas/MemoryPage.html` を貼り付け・ファイル名は必ず `MemoryPage`）

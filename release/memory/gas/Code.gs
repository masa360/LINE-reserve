/**
 * ============================================================
 * メモリー専用 GAS（予約システムと完全分離）
 * ============================================================
 *
 * doPost:
 * - LINE webhook（画像メッセージ）を受け取り
 * - Google Drive に保存し、ログを Spreadsheet に記録
 *
 * doGet:
 * - action=photos の場合: lineUserId に紐づく写真一覧を JSON で返す
 * - それ以外の場合: LIFF画面（MemoryPage.html）を返す
 *
 * 必須スクリプトプロパティ:
...
function fetchLineImageBlob_(messageId) {
  ...
  if (code < 200 || code >= 300) throw new Error('画像取得失敗: ' + code);
  return res.getBlob();
}
